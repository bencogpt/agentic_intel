...
const { getOrFetchSession } = require('../sessions/store');
const { loadDocument } = require('../storage/documents');

const router = express.Router();

router.get('/:sessionId', async (req, res) => {
  // Always use getOrFetchSession — bypasses metadata-only stubs from startup cache
  ...
  // Load document text from storage if not in memory (different Cloud Run instance)
